...
class AgentGuardRuntime:
    def __init__(self, registry: ToolRegistry, planner, store, scanner: SecurityScanner | None = None, policy: PolicyDocument | None = None) -> None:
    ...
    def _agent_decision(self, state: AgentState) -> AgentState:
        ...
        decision = self.planner.plan(system_prompt=system_prompt, user_message=state["message"])
        return {**state, "tool_name": decision.get("tool_name"), "tool_args": decision.get("tool_args", {}), "answer": decision.get("answer", "")}
    ...
    @staticmethod
    def _route_after_scan(state: AgentState) -> Literal["scan_output", "agent_decision"]:
        return "scan_output" if state.get("blocked") else "agent_decision"

    @staticmethod
    def _route_after_policy(state: AgentState) -> Literal["scan_output", "execute_tool"]:
        if state.get("requires_approval") or not state.get("tool_name"):
            return "scan_output"
    ...
    def _build_graph(self):
        ...
        graph.add_conditional_edges("scan_input", self._route_after_scan, {"agent_decision": "agent_decision", "scan_output": "scan_output"})
        graph.add_edge("agent_decision", "policy_guard")
        graph.add_conditional_edges("policy_guard", self._route_after_policy, {"execute_tool": "execute_tool", "scan_output": "scan_output"})
        graph.add_edge("execute_tool", "scan_output")
